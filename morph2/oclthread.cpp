...
void oclThread::setOpenCLBackend(EOpenCLBackend backend)
{
	if(!isRunning())
...
void oclThread::run()
{
	if(backend == OB_Images)
		startWithImages();
	else 
		startWithBuffers();
}

void oclThread::startWithImages()
{
	QCLContext* ctx;
	...
	QCLWorkSize localWorkSize
		(conf.workgroupSizeX, conf.workgroupSizeY);
	...
	bayerFilter.setLocalWorkSize(localWorkSize);
	...
	morphFilter.setLocalWorkSize(localWorkSize);
	...
	hitmissFilter.setLocalWorkSize(localWorkSize);
...
void oclThread::startWithBuffers()
{
	QCLContext* ctx = new QCLContext;
	initContext(ctx);
	...
	oclBayerFilterBuffer bayerFilter(ctx);
	success = ctx->lastError() == CL_SUCCESS;
	if(!success)
	{
		emit openCLInitialized(false);
	...
	// Blok filtracji morfologicznej
	oclMorphFilterBuffer morphFilter
		(ctx, conf.erode_1d.toAscii().constData(),
		 conf.dilate_1d.toAscii().constData(), 
		 conf.gradient_1d.toAscii().constData(),
		 conf.subtract_1d.toAscii().constData());
	if(!success)
	{
		emit openCLInitialized(false);
		return;
	...
	// Blok filtracji morfologicznej typu Hit-Miss
	oclMorphHitMissFilterBuffer hitmissFilter
		(ctx, conf.hitmiss_1d == "local", 
		 conf.atomicCounters);
	if(!success)
	{
		emit openCLInitialized(false);
	...
	while(true)
	{
		...
		int gpuWidth = oclUtils::roundUp(item.src.cols, conf.workgroupSizeX);
		int gpuHeight = oclUtils::roundUp(item.src.rows, conf.workgroupSizeY);

		QSize size(gpuWidth, gpuHeight);
		QRect roi(0, 0, item.src.cols, item.src.rows);

		// Utworz obraz na urzadzeniu w postaci bufora
		QCLBuffer holder = ctx->createBufferDevice
			(gpuWidth * gpuHeight,
			 QCLMemoryObject::ReadOnly);

		// Skopiuj do niego dane
		QCLEvent evt = holder.writeRectAsync
			(roi, item.src.data, gpuWidth, 
			 item.src.cols, QCLEventList());
		evt.waitForFinished();
		...
		{
			bayerFilter.setBayerFilter(item.bc);
			bayerFilter.setSourceImage(holder, size, roi);
		...
		{	
			...
			else
			{
				morphFilter.setMorphologyOperation(item.op);
				morphFilter.setStructuringElement(item.se);
				morphFilter.setSourceImage(holder, size, roi);
		...
		//if(!glInterop)
		{
			...
			int format = CV_8U;
			pitem.dst = cv::Mat
				(item.src.size(), format, cv::Scalar(1));

			evt = holder.readRectAsync
				(roi, pitem.dst.ptr<uchar>(), gpuWidth,
				 pitem.dst.cols, QCLEventList());
