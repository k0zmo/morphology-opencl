...
oclBayerFilterBuffer::oclBayerFilterBuffer(QCLContext* ctx)
	: oclFilterBuffer(ctx)
	, d_kernel(nullptr)
{
	printf("\n*---- Bayer filter initialization ----*\n");
	...
	// Wczytaj program
	QCLProgram program = ctx->createProgramFromSourceFile("kernels/1d/bayer.cl");
	if(program.isNull() ||
...
qreal oclBayerFilterBuffer::run()
{
	if(!d_src)
		return 0.0;
	...
	d_kernel->setArg(1,  d_dst);
	d_kernel->setArg(2, 
		QPoint(d_size.width(), d_size.height()));
...
void oclBayerFilterBuffer::setBayerFilter(cvu::EBayerCode bc)
{
	if(bc == cvu::BC_None)
		d_kernel = nullptr;
	else
